...
`value` 根据 kind 编码：money 为非负十进制字符串，unit 为 ISO 4217 三字母币种；traffic 为非负整数字节字符串，unit 固定为 `byte`；expiry 为带时区的 ISO 8601 时间字符串，unit 固定为 `datetime`。金额不得按不同币种求和。流量显示 GB/GiB 时在展示层转换。无限流量与缺失字段不是零：首版不编码“无限”，适配器遇到无限额度时应扩展协议后再接入。
